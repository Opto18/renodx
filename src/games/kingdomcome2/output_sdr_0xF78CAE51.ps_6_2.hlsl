...
float4 main(
  ...
  float vignette_mask = renodx::math::PowSafe(saturate(_17.x), vignette_strength);
  float _70 = (8333.3330078125f / exp2(min(max((log2(_23.y * 3030.30322265625f) - ((HDREyeAdaptation.z * 0.5f) * (min(max((log2((_23.y * 10000.0f) + 1.0f) * 0.3010300099849701f), 0.10000000149011612f), 5.199999809265137f) + -3.0f))), HDREyeAdaptation.x), HDREyeAdaptation.y) - HDREyeAdaptation.w)) * vignette_mask;
  float _87 = ((saturate(HDRBloomColor.x) * bloom_strength * (_19.x - _45)) + _45) * _70;
  float _88 = ((saturate(HDRBloomColor.y) * bloom_strength * (_19.y - _46)) + _46) * _70;
  float _89 = ((saturate(HDRBloomColor.z) * bloom_strength * (_19.z - _47)) + _47) * _70;
  ...
  float _274 = sin(dot(float2(SV_Position.x, SV_Position.y), float2(34.483001708984375f, 89.63700103759766f)));
  float _284 = sin(dot(float2((SV_Position.x + 0.5788999795913696f), (SV_Position.y + 0.5788999795913696f)), float2(34.483001708984375f, 89.63700103759766f)));
  SV_Target.x = saturate((HDRUserModification.y * exp2(log2(((((1.0f - (((1.0f - _217) * 2.0f) * _251)) - _259) * select((_217 < 0.5f), 0.0f, 1.0f)) + _259) + (((frac(_274 * 29156.4765625f) + -0.5f) + frac(_284 * 29156.4765625f)) * 0.0019607844296842813f)) * HDRUserModification.x)) + HDRUserModification.z);
  SV_Target.y = saturate((HDRUserModification.y * exp2(log2(((((1.0f - (((1.0f - _218) * 2.0f) * _251)) - _261) * select((_218 < 0.5f), 0.0f, 1.0f)) + _261) + (((frac(_274 * 38273.5625f) + -0.5f) + frac(_284 * 38273.5625f)) * 0.0019607844296842813f)) * HDRUserModification.x)) + HDRUserModification.z);
  SV_Target.z = saturate((HDRUserModification.y * exp2(log2(((((1.0f - (((1.0f - _219) * 2.0f) * _251)) - _263) * select((_219 < 0.5f), 0.0f, 1.0f)) + _263) + (((frac(_274 * 47843.75390625f) + -0.5f) + frac(_284 * 47843.75390625f)) * 0.0019607844296842813f)) * HDRUserModification.x)) + HDRUserModification.z);
  SV_Target.w = 1.0f;
  return SV_Target;
}
