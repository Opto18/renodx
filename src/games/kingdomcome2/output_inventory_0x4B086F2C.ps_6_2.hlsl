...
float4 main(
  ...
  float _155 = 8333.3330078125f / exp2(min(max((log2(_108.y * 3030.30322265625f) - ((HDREyeAdaptation.z * 0.5f) * (min(max((log2((_108.y * 10000.0f) + 1.0f) * 0.3010300099849701f), 0.10000000149011612f), 5.199999809265137f) + -3.0f))), HDREyeAdaptation.x), HDREyeAdaptation.y) - HDREyeAdaptation.w);
  float _156 = lerp(1.0f, pow(max(saturate(_102.x), 0.0001f), max(1.0f, CUSTOM_VIGNETTE)), min(1.0f, CUSTOM_VIGNETTE));
  float _172 = ((saturate(HDRBloomColor.x) * CUSTOM_BLOOM * (_104.x - _130)) + _130) * _155 * _156;
  float _173 = ((saturate(HDRBloomColor.y) * CUSTOM_BLOOM * (_104.y - _131)) + _131) * _155 * _156;
  float _174 = ((saturate(HDRBloomColor.z) * CUSTOM_BLOOM * (_104.z - _132)) + _132) * _155 * _156;
  ...
  /* float _337 = saturate(exp2(log2((((1.0f - (((1.0f - _272) * 2.0f) * _306)) - _314) * select((_272 < 0.5f), 0.0f, 1.0f)) + _314) * 2.200000047683716f));
  float _338 = saturate(exp2(log2((((1.0f - (((1.0f - _273) * 2.0f) * _306)) - _316) * select((_273 < 0.5f), 0.0f, 1.0f)) + _316) * 2.200000047683716f));
  float _339 = saturate(exp2(log2((((1.0f - (((1.0f - _274) * 2.0f) * _306)) - _318) * select((_274 < 0.5f), 0.0f, 1.0f)) + _318) * 2.200000047683716f));
   */
  ...
  if (!RENODX_TONE_MAP_TYPE) {
    float _499 = mad(0.04330600053071976f, _489, mad(0.3292919993400574f, _487, (_485 * 0.6274020075798035f)));
    float _502 = mad(0.011359999887645245f, _489, mad(0.919543981552124f, _487, (_485 * 0.06909500062465668f)));
    float _505 = mad(0.8955780267715454f, _489, mad(0.08802799880504608f, _487, (_485 * 0.016394000500440598f)));
    float _508 = ((HDRDisplayParams.x - HDRTonemappingParams.y) * HDRTonemappingParams.z) / HDRTonemappingParams.x;
    float _509 = _508 + HDRTonemappingParams.y;
    float _513 = HDRDisplayParams.x - ((_508 * HDRTonemappingParams.x) + HDRTonemappingParams.y);
    float _515 = _499 / HDRTonemappingParams.y;
    float _516 = _502 / HDRTonemappingParams.y;
    float _517 = _505 / HDRTonemappingParams.y;
    float _518 = saturate(_515);
    float _519 = saturate(_516);
    float _520 = saturate(_517);
    float _528 = (_518 * _518) * (3.0f - (_518 * 2.0f));
    float _530 = (_519 * _519) * (3.0f - (_519 * 2.0f));
    float _532 = (_520 * _520) * (3.0f - (_520 * 2.0f));
    float _539 = select((_499 < _509), 0.0f, 1.0f);
    float _540 = select((_502 < _509), 0.0f, 1.0f);
    float _541 = select((_505 < _509), 0.0f, 1.0f);
    float _555 = (-0.0f - ((HDRDisplayParams.x * HDRTonemappingParams.x) / _513)) / HDRDisplayParams.x;
    SV_Target.x = (((((1.0f - _528) * HDRTonemappingParams.y) * (pow(_515, HDRTonemappingParams.w))) + ((_528 - _539) * (lerp(HDRTonemappingParams.y, _499, HDRTonemappingParams.x)))) + ((HDRDisplayParams.x - (exp2(((_499 - _509) * 1.4426950216293335f) * _555) * _513)) * _539));
