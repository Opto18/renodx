...
float4 main(
  ...
  float _156 = 8333.3330078125f / exp2(min(max((log2(_109.y * 3030.30322265625f) - ((HDREyeAdaptation.z * 0.5f) * (min(max((log2((_109.y * 10000.0f) + 1.0f) * 0.3010300099849701f), 0.10000000149011612f), 5.199999809265137f) + -3.0f))), HDREyeAdaptation.x), HDREyeAdaptation.y) - HDREyeAdaptation.w);
  float _157 = lerp(1.0f, pow(max(saturate(_103.x), 0.0001f), max(1.0f, CUSTOM_VIGNETTE)), min(1.0f, CUSTOM_VIGNETTE));
  float _173 = ((saturate(HDRBloomColor.x) * CUSTOM_BLOOM * (_105.x - _131)) + _131) * _156 * _157;
  float _174 = ((saturate(HDRBloomColor.y) * CUSTOM_BLOOM * (_105.y - _132)) + _132) * _156 * _157;
  float _175 = ((saturate(HDRBloomColor.z) * CUSTOM_BLOOM * (_105.z - _133)) + _133) * _156 * _157;
  ...
  if (!RENODX_TONE_MAP_TYPE) {
    float _555 = mad(0.04330600053071976f, _545, mad(0.3292919993400574f, _543, (_541 * 0.6274020075798035f)));
    float _558 = mad(0.011359999887645245f, _545, mad(0.919543981552124f, _543, (_541 * 0.06909500062465668f)));
    float _561 = mad(0.8955780267715454f, _545, mad(0.08802799880504608f, _543, (_541 * 0.016394000500440598f)));
    float _564 = ((HDRDisplayParams.x - HDRTonemappingParams.y) * HDRTonemappingParams.z) / HDRTonemappingParams.x;
    float _565 = _564 + HDRTonemappingParams.y;
    float _569 = HDRDisplayParams.x - ((_564 * HDRTonemappingParams.x) + HDRTonemappingParams.y);
    float _571 = _555 / HDRTonemappingParams.y;
    float _572 = _558 / HDRTonemappingParams.y;
    float _573 = _561 / HDRTonemappingParams.y;
    float _574 = saturate(_571);
    float _575 = saturate(_572);
    float _576 = saturate(_573);
    float _584 = (_574 * _574) * (3.0f - (_574 * 2.0f));
    float _586 = (_575 * _575) * (3.0f - (_575 * 2.0f));
    float _588 = (_576 * _576) * (3.0f - (_576 * 2.0f));
    float _595 = select((_555 < _565), 0.0f, 1.0f);
    float _596 = select((_558 < _565), 0.0f, 1.0f);
    float _597 = select((_561 < _565), 0.0f, 1.0f);
    float _611 = (-0.0f - ((HDRDisplayParams.x * HDRTonemappingParams.x) / _569)) / HDRDisplayParams.x;
    SV_Target.x = (((((1.0f - _584) * HDRTonemappingParams.y) * (pow(_571, HDRTonemappingParams.w))) + ((_584 - _595) * (lerp(HDRTonemappingParams.y, _555, HDRTonemappingParams.x)))) + ((HDRDisplayParams.x - (exp2(((_555 - _565) * 1.4426950216293335f) * _611) * _569)) * _595));
    SV_Target.y = (((((1.0f - _586) * HDRTonemappingParams.y) * (pow(_572, HDRTonemappingParams.w))) + ((_586 - _596) * (lerp(HDRTonemappingParams.y, _558, HDRTonemappingParams.x)))) + ((HDRDisplayParams.x - (exp2(((_558 - _565) * 1.4426950216293335f) * _611) * _569)) * _596));
    SV_Target.z = (((((1.0f - _588) * HDRTonemappingParams.y) * (pow(_573, HDRTonemappingParams.w))) + ((_588 - _597) * (lerp(HDRTonemappingParams.y, _561, HDRTonemappingParams.x)))) + ((HDRDisplayParams.x - (exp2(((_561 - _565) * 1.4426950216293335f) * _611) * _569)) * _597));
  } else {
    SV_Target.rgb = float3(_541, _543, _545);
